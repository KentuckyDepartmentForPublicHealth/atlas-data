anno<-read.csv("/Users/amistry/Library/CloudStorage/OneDrive-uoflhealth/Atlas/Atlas_data.csv", header=T, row.names = 1, stringsAsFactors = T)
anno$filename<-rownames(anno)
anno$Diagnosis<-as.character(anno$Diagnosis)
anno$Tumor<-anno$Train.diagnosis_bc
...
dismat<-readRDS("dismat_core_bc.RDS") #'core' dataset
dismatfull<-readRDS("dismat_full_bc.RDS") #includes 'core' and 'test' datasets
...
mean(dclass[["byClass"]][ , "F1"]) #macro-F1

library(pROC)
multiclass.roc(as.numeric(as.factor(mat$orig)), as.numeric(as.factor(mat$predict)))
...
e<-prettyConfused(mat$orig, mat$predict)+coord_cartesian(clip='off')+
  annotate("text", x=1, y=54, label= "Euclidean Classifier: Overall Accuracy: 99.26% [95% CI: 98.99% to 99.48%]", hjust = 0, fontface = "bold")
...
mean(tclass[["byClass"]][ , "F1"]) #macro-F1

library(pROC)
multiclass.roc(as.numeric(as.factor(mattest$orig)), as.numeric(as.factor(mattest$predict)))

library(grid)
#x_grob <- rectGrob(x=1:52, y=0, gp=gpar(color='black', fill=rainbow(52), alpha=0.75))
#y_grob <- rectGrob(y=1:52, x=0, gp=gpar(color='black', fill=rainbow(52), alpha=0.75))
t<-prettyConfused(mattest$orig, mattest$predict)+coord_cartesian(clip='off')+
  annotate("text", x=1, y=54, label= "t-SNE-Based Classifier: Overall Accuracy: 99.22% [95% CI: 98.94% to 99.44%]", hjust = 0, fontface = "bold")
#  annotation_custom(grob=x_grob, xmin=0, xmax=1, ymin=-3.75, ymax=3.75)+
#  annotation_custom(grob=y_grob, xmin=-3.75, xmax=3.75, ymin=0, ymax=1)
#750x750
...
#GBM
pred<-readRDS("gbm_predict_bc.RDS")
anno<-read.csv("Atlas_data.csv",header = T)
anno<-anno[anno$Train.diagnosis_bc!="#N/A",]

g<-prettyConfused(as.factor(anno$Train.diagnosis_bc), pred)+coord_cartesian(clip='off')+
  annotate("text", x=1, y=54, label= "LightGBM Classifier: Overall Accuracy: 99.94% [95% CI: 99.83% to 99.99%]", hjust = 0, fontface = "bold")

gbmconf<-readRDS("gbm_conf_bc.RDS")
mean(gbmconf[["byClass"]][ , "F1"]) #macro-F1
library(pROC)
multiclass.roc(as.numeric(as.factor(anno$Train.diagnosis_bc)), as.numeric(as.factor(pred)))

#RF
pred<-readRDS("rf_predict_bc.RDS")
anno<-read.csv("Atlas_data.csv",header = T)
anno<-anno[anno$Train.diagnosis_bc!="#N/A",]
r<-prettyConfused(as.factor(make.names(anno$Train.diagnosis_bc)), pred)+coord_cartesian(clip='off')+
  annotate("text", x=1, y=54, label= "Random Forest Classifier: Overall Accuracy: 100% [95% CI: 99.93% to 100%]", hjust = 0, fontface = "bold")

rfconf<-readRDS("rf_conf_bc.RDS")
mean(rfconf[["byClass"]][ , "F1"]) #macro-F1

library(pROC)
multiclass.roc(as.numeric(as.factor(make.names(anno$Train.diagnosis_bc))), as.numeric(as.factor(pred)))
