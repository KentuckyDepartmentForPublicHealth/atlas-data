...
d<-read.csv("Atlas_data.csv", header=T, stringsAsFactors = T)
...
mb<-ggsurvplot(fit, risk.table = F, legend='none', xlab="Time (Months)")$plot+
  ...
  annotate("text", x=78, y=.79, label= "MB-GP4 (n=52)",hjust = 0)+
  annotate("text", x=78, y=.70, label= "MB-GP3 (n=29)",hjust = 0)+
  annotate("text", x=78, y=.55, label= "MB-SHH (n=25)",hjust = 0)

d<-read.csv("Atlas_data.csv", header=T)
d<-d[grep("^AT",d$Final.diagnosis),]
fit<- survfit(Surv(as.numeric(Overall.survival..months.), Vital.status..1.dead..0.alive.) ~ Final.diagnosis, data = d)
at<-ggsurvplot(fit, risk.table = F, legend='none', xlab="Time (Months)",xlim=c(0,50),break.x.by=10)$plot+
  ...
  annotate("text", x=25, y=.34, label= "AT/RT-SHH (n=12)",hjust = 0)+
  annotate("text", x=25, y=.08, label= "AT/RT-TYR (n=7)",hjust = 0)

d<-read.csv("Atlas_data.csv", header=T)
d<-d[d$Final.diagnosis=="ETMR"|d$Final.diagnosis=="HGNET-BCOR"|d$Final.diagnosis=="NB-FOXR2",]
fit<- survfit(Surv(as.numeric(Overall.survival..months.), Vital.status..1.dead..0.alive.) ~ Final.diagnosis, data = d)
em<-ggsurvplot(fit, risk.table = F, legend='none', xlab="Time (Months)")$plot+
  annotate("text", x=35, y=.93, label= "NB-FOXR2 (n=7)",hjust = 0)+
  annotate("text", x=35, y=.56, label= "HGNET-BCOR (n=8)",hjust = 0)+
  annotate("text", x=35, y=.17, label= "ETMR (n=9)",hjust = 0)

d<-read.csv("Atlas_data.csv", header=T)
d<-d[d$Final.diagnosis=="Neuroblastoma",]
...
d<-read.csv("Atlas_data.csv", header=T)
d<-d[d$Final.diagnosis=="YAP"|d$Final.diagnosis=="PFB"|d$Final.diagnosis=="PFA"|d$Final.diagnosis=="RELA",]
fit<- survfit(Surv(as.numeric(Overall.survival..months.), Vital.status..1.dead..0.alive.) ~ Final.diagnosis, data = d)
ep<-ggsurvplot(fit, risk.table = T, legend='none', xlab="Time (Months)")$plot+
  annotate("text", x=90, y=1, label= "PFB (n=19)/YAP (n=6)",hjust = 0)+
  annotate("text", x=105, y=.51, label= "PFA (n=148)",hjust = 0)+
  annotate("text", x=93, y=.22, label= "RELA (n=18)",hjust = 0)

d<-read.csv("Atlas_data.csv", header=T)
d<-d[d$Final.diagnosis=="Meningioma",]
...
d<-read.csv("Atlas_data.csv", header=T)
d<-d[d$Final.diagnosis=="Diffuse Glioma"|d$Final.diagnosis=="IDH Mutant"|d$Final.diagnosis=="G34 Mutant"|d$Final.diagnosis=="Midline",]
d<-d[d$Dataset.ID!="GSE53733",]
fit<- survfit(Surv(as.numeric(Overall.survival..months.), Vital.status..1.dead..0.alive.) ~ Final.diagnosis, data = d)
dg<-ggsurvplot(fit, risk.table = F, legend='none', xlab="Time (Months)")$plot+
  annotate("text", x=45, y=.90, label= "IDH mutant (n=426)",hjust = 0)+
  annotate("text", x=130, y=.65, label= "G34 mutant (n=15)",hjust = 0,color="#0CB702")+
  annotate("text", x=77, y=.11, label= "Diffuse glioma\n             (n=980)",hjust = 0)+
  annotate("text", x=130, y=.45, label= "Midline (n=49)",hjust = 0, color="#c77cff")
...
dgg<-ggsurvplot(fit, risk.table = F, legend='none', xlab="Time (Months)")$plot+
  ...
  annotate("text", x=145, y=.25, label= "Grade 3 (n=419)",hjust = 0,color="#0CB702")+
